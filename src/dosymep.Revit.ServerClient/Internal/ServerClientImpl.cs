...
internal class ServerClientImpl : IServerClient {
    ...
    public async Task<Stream> GetModelThumbnailAsync(string modelPath, int width, int height,
        ...
        HttpResponseMessage response =
            await _httpClient.Get($"{modelPath}/thumbnail?width={width}&height={height}", cancellationToken);
        return await response.Content.ReadAsStreamAsync();
    }

    /// <inheritdoc />
    public async Task<ProjectInfo>
        GetProjectInfoAsync(string modelPath, CancellationToken cancellationToken = default) {
        if(string.IsNullOrEmpty(modelPath)) {
            throw new ArgumentException("Value cannot be null or empty.", nameof(modelPath));
        }
        ...
        List<ParamInfoItem> items =
            _jsonSerialization.Deserialize<List<ParamInfoItem>>(await response.Content.ReadAsStringAsync());
    ...
    public async Task CopyOrMoveAsync(string sourceObjectPath, string destinationObjectPath, PasteAction pasteAction,
        ...
        await _httpClient.Post(
            $"{sourceObjectPath}?destinationObjectPath={destinationObjectPath}&pasteAction={pasteAction}&replaceExisting={replaceExisting}",
            cancellationToken);
    }
