...
public abstract class RevitAddinItem {
    ...
    ///     Users have the option to ignore that error in the future. This property is used to represent the user's choice to
    ///     ignore an error.
    ...
    protected static bool IsTypeInheritInterface(Type type, string interfaceFullName) {
        ...
        return interfaceType?.FullName?.Equals(interfaceFullName) == true
               && (assemblyName?.Name.Equals(AssemblyRevitApi) == true ||
                   assemblyName?.Name.Equals(AssemblyRevitApiUi) == true);
    }
