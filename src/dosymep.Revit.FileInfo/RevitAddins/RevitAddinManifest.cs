...
public class RevitAddinManifest {
    ...
    /// </summary>
    public List<RevitAddinCommand> AddinCommands { get; } = new();

    /// <summary>
    ///     The collection of all external applications in the manifest.
    /// </summary>
    public List<RevitAddinApplication> AddinApplications { get; } = new();

    /// <summary>
    ///     The collection of all external DB applications in the manifest.
    /// </summary>
    public List<RevitAddinDBApplication> AddinDBApplications { get; } = new();
    ...
    public static RevitAddinManifest GetAddinManifest(string fullFileName) {
        ...
        if(documentElement == null
           || !(documentElement.Name?.Equals(RevitAddInTag) == true
            ...
            throw new ArgumentException(
                $"The \"{documentElement?.Name ?? "null"}\" root tag is not valid. Tag should be {RevitAddInTag} or {RevitAddInsTag}.",
                nameof(fullFileName));
        }
        ...
        RevitAddinManifest addinManifest = new() {FullName = fullFileName};
        foreach(XmlNode addinElement in addinElements) {
            if(!addinElement.Name.Equals(AddInTag)) {
                throw new ArgumentException($"The addin element is not valid. Tag should be {AddInTag}",
                    nameof(fullFileName));
            }

            if(addinElement.Attributes == null || addinElement.Attributes.Count == 0 ||
               addinElement.Attributes[RevitAddinItem.AddInTypeTag] == null) {
                throw new ArgumentException(
                    $"The addin element does not have valid type. Tags should be {AddInCommandTag} or {AddInApplicationTag} or {AddInDBApplicationTag}",
                    nameof(fullFileName));
            }
            ...
            if(attribute.Value.Equals(AddInCommandTag)) {
                ...
                addinManifest.AddinApplications.Add(
                    RevitAddinApplication.CreateAddinApplication(addinElement, addinManifest));
            } else if(attribute.Value.Equals(AddInDBApplicationTag)) {
                addinManifest.AddinDBApplications.Add(
                    RevitAddinDBApplication.CreateAddinDBApplication(addinElement, addinManifest));
            } else {
                throw new NotSupportedException(
                    $"The {attribute.Value} addin type is not valid. Addin type should be {AddInCommandTag} or {AddInApplicationTag} or {AddInDBApplicationTag}.");
            }
        }
    ...
    public static RevitAddinManifest CreateAddinManifest(string assemblyPath) {
        ...
        RevitAddinManifest manifest = new() {FullName = Path.ChangeExtension(assemblyPath, AddinFileExt)};
